...
```{r}
f3 <- ergm(net ~ edges+transitiveties+mutual+cyclicalties+dgwdsp(1, fixed = TRUE, cutoff = 7, type = c("OSP"))+edgecov(distance)+edgecov(net2016)+nodeocov("alsha_occ")+nodematch("clan")+diff("pop14")+diff("urban")+nodematch("region"))
summary(f3)
```
```{r}
f4 <- ergm(net ~ edges+transitiveties+mutual+cyclicalties+dgwdsp(1, fixed = TRUE, cutoff = 7, type = c("OSP"))+edgecov(distance)+edgecov(net2016)+nodeocov("alsha_occ")+diff("urbanrural")+nodematch("region")+nodeocov("pdi_m")+nodeicov("pdi_m")+nodeocov("chirps_m")+nodeicov("chirps_m")+nodeocov("norain_m")+nodeicov("norain_m"))
summary(f4)
```

```{r}
f5 <- ergm(net ~ edges+transitiveties+mutual+cyclicalties+dgwdsp(1, fixed = TRUE, cutoff = 7, type = c("OSP"))+edgecov(distance)+edgecov(net2016)+nodeocov("alsha_occ")+diff("urbanrural")+nodematch("region")+nodeocov("tdi_m")+nodeicov("tdi_m")+nodeocov("temp_m")+nodeicov("temp_m"))
summary(f5)
```

```{r}
f6 <- ergm(net ~ edges+transitiveties+mutual+cyclicalties+dgwdsp(1, fixed = TRUE, cutoff = 7, type = c("OSP"))+edgecov(distance)+edgecov(net2016)+nodeocov("alsha_occ")+diff("urbanrural")+nodematch("region")+nodeocov("vdi_m")+nodeicov("vdi_m")+nodeocov("ndvi_m")+nodeicov("ndvi_m"))
summary(f6)
```